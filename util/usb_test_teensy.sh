...
find_sysfs_teensy() {
	local d vendor product devmaj devmin node node_major node_minor perms found
	for d in /sys/bus/usb/devices/*; do
		...
		if [[ -f "$d/dev" ]]; then
			...
			for node in /dev/bus/usb/*/*; do
				[[ -e "$node" ]] || continue
				read -r node_major node_minor < <(stat -c '%t %T' "$node")
				(( 16#$node_major == devmaj && 16#$node_minor == devmin )) || continue
				found=1
